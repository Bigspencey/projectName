...
var get_url = window.location.href, reportSuite;
if(get_url.indexOf("triafhsstg.ashleyfurniturehomestore.com")!==-1 || get_url.indexOf("triafhsstgcms.ashleyfurniturehomestore.com") !== -1){
 reportSuite = "ashleyhomestoretriafhsstg"; 
}else if(get_url.indexOf("www.ashleyfurniturehomestore.com")!==-1 || get_url.indexOf("www.meublesashley.com") !== -1){
 reportSuite = "ashleyhomestoreprod"
}else {
  reportSuite = "ashleyhomestoredtmdev";
}
...
/* adobe added this plugin code */
s_getLoadTime();

s.debugTracking=false
s.trackingServer="omni.ashleyfurniturehomestore.com"
s.trackingServerSecure="somni.ashleyfurniturehomestore.com"

s.visitorNamespace="ashleyhomestores";
s.linkTrackVars="events";
s.linkTrackEvents="event35";

s.usePlugins=true;
	function s_doPlugins(s) {
		/* Adoeb added this code to capture page load times into an eVar and event */ 
    s.eVar19=s_getLoadTime();
    if(s_getLoadTime())s.events=s.apl(s.events,'event60='+s_getLoadTime(),',',1);
    
    s.eVar75 = document.location.href;
    ordConf = document.location.search;
    
    if(window.location.hostname.indexOf("www.meublesashley.com") > -1) {
       s.eVar65 = "French";
    }else{s.eVar65 = "English";}
    
    if(s.pageName == 'afhs:Home:cart:Checkout') {
      s.abort = true;
    }else{s.abort = false;}
		
    if(s.pageName == 'afhs:Home:Errors:404') {
      s.pageType = "errorPage";
      s.pageName = "";
    }
       
    if(window.location.href.indexOf("/furniture-stores/") > -1) {
      s.prop19 = $("#contentCalendarStore").html().split("<br>")[1];
    }
    
    /* Add calls to plugins here */
    /* New/Returning Visitor Tracking */
    s.eVar58=s.getNewRepeat(30,'s_getNewRepeat');
...
2019:'3/10,11/3'}
   	...
   	var tp = s.getTimeParting('n','-6');  // Set Time Parting
    s.eVar59 = tp;
    
    if(s.eVar59.indexOf("Monday") > -1 || s.eVar59.indexOf("Tuesday") > -1 || s.eVar59.indexOf("Wednesday") > -1 || s.eVar59.indexOf("Thursdayday") > -1 || s.eVar59.indexOf("Friday") > -1) {
      s.eVar60 = "Weekday";
    }else{ s.eVar60 = "Weekend"; }
    
    /*Campaign Stacking*/ 
    s.eVar61=s.crossVisitParticipation(s.campaign,"s_v61","30","5",">",'eventX', 1);
    
		/*Internal Search Deduping*/
		...
		/*Internal Campaign Variable Setting*/
		if(!s.eVar49)
			s.eVar49=s.getQueryParam('icid');
    ...
    if(window.location.href.indexOf('?checkout=1#shipping') > -1) {
      s.pageName='afh:checkout:login';
 }   
...
/* * s.join: 1.0 - Joins an array into a string - Used with Cross-Visit Participation Plug-In*/ 
...
/* * Plugin Utility: Replace v1.0 - Used with Cross-Visit Participation Plug-In*/ 
s.repl=new Function("x","o","n","" +"var i=x.indexOf(o),l=n.length;while(x&&i>=0){x=x.substring(0,i)+n+x." +"substring(i+o.length);i=x.indexOf(o,i+l)}return x");

/*
 * Plugin: getTimeParting 3.4 - Set timeparting values based on time zone (15 min)
 */
