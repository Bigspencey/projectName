 ...
 * - Products
 *   - Added capabilities for displaying PNGs.
 *	 - Added options for linking directly to PDPs and opening in new window
 */

/* Fix issues old browsers not supporting console methods */
...
/* Inclusion of SWFObject for loading flash-based video player */
...
function YouTube() {
	...
	this._defaults = {
		...
		height : "360",
		width : "640",
		returnCode : false, // Return the HTML directly instead of outputting to the page
		annotate : false // Toggle Youtube annotations
	}
	this.init = function(id) {
		...
		var yt = ('<iframe width="'+this._defaults.width+'" height="'+this._defaults.height+'" src="http://www.youtube.com/embed/'+this._defaults.id+'?rel=0&wmode=opaque'+ivpolicy+'" frameborder="0" allowfullscreen></iframe>');
		if(/msie/.test(navigator.userAgent.toLowerCase())) {
			yt = ('<object width="'+this._defaults.width+'" height="'+this._defaults.height+'"><param name="movie" value="http://www.youtube.com/v/'+this._defaults.id+'?version=3&hl=en_US&rel=0'+ivpolicy+'"></param><param name="allowFullScreen" value="true"></param><param name="wmode" value="opaque"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/'+this._defaults.id+'?version=3&hl=en_US&rel=0'+ivpolicy+'" type="application/x-shockwave-flash" width="'+this._defaults.width+'" height="'+this._defaults.height+'" wmode="opaque" allowscriptaccess="always" allowfullscreen="true"></embed></object>');
		}
		if(typeof this.callback == "function") this.callback(); else {};
...
$.fn.videoGallery = function(options, callback) {
	$.videoGallery = new VideoGallery(); // Video gallery instance
...
function Products() {
	...
	this.init = function() {
		...
		//Filter for plugin specific properties vs properties to search by
		for(prop in this._defaults) {
			if(this._defaults[prop] != "" && prop != "reviewNum" && prop != "imageSize" && prop != "grpSize" && prop != "baseURL" && prop != "skuList" && prop != "imageDomain" && prop != "jpgImageDomain" && prop != "itemClass" && prop != "groupClass" && prop != "lazyLoad" && prop != "linkPDP" && prop != "png" && prop != "newwindow") {
				filters.push(prop + '=' + this._defaults[prop]);
			}
		}
		
		//Set image paths if not specified.
		//Scene 7
		if(this._defaults.imageDomain === ''){		
			this._defaults.imageDomain = window.location.protocol+"//"+window.location.host.replace(window.location.host.split('.')[0], 'images');
		}
		//Regular
		if(this._defaults.jpgImageDomain === ''){
			this._defaults.jpgImageDomain = window.location.protocol+"//"+window.location.host;
		}
		...
		$.ajax({
			...
			for (var l = 0; l < feed.RECORDS.length; l++) {
				if(typeof window['debug'] !== 'undefined'){
					console.log(escape(feed.RECORDS[l].PROPERTIES.P_ModelName));
				}
				...
				for(var count = 0; count < obj._defaults.grpSize; count++) {
					if(l < feed.RECORDS.length) {
						...
						//Set link target
						if(obj._defaults.newwindow){
							target = 'target="_blank"';
						}else{
							target = '';
						}
						//Setup for linking directly to the PDP
						if(!obj._defaults.linkPDP){
							html += "<a href=\"" + feed.RECORDS[l].PDPURL + "\" class=\"product_cell cell-"+(l+1)+"\" title=\"" + info.P_ModelName + "\" onclick=\"return false;\">";
						}else{
							html += "<a href=\"" + feed.RECORDS[l].PDPURL + "\" class=\"product_cell cell-"+(l+1)+"\" title=\"" + info.P_ModelName + "\" "+target+" >";
						}
						//Setup lazyload
						if(obj._defaults.lazyLoad) {
							try {
						...
						var h,w,failPng=false;
						/* Break up size item for Scene 7 dimansions */
						if(obj._defaults.png){
							switch(obj._defaults.imageSize[1]){
								case 'c':
									h=50;
									w=50;
								break;
						...
						if(obj._defaults.png){
							 //Test if the image actually exists first
							var ts = new Date();
							var testIMG = $('<img />')
								.load(function(){ failPng=false; })
								.error(function(){ failPng=true; })
								.attr('src', obj._defaults.imageDomain+"/is/image/EBFL2/"+ feed.RECORDS[l].PROPERTIES.P_StyleSKU + "?fmt=png-alpha&wid="+w+"&hei="+h+"&ts="+ts.getMilliseconds());
								
							if(failPng){
								//Fall back to old product images if no PNG exists
								if(obj._defaults.lazyLoad) {
									html += "<img src=\"data:image/gif;base64,R0lGODlhAQABAID/AMDAwAAAACH5BAEAAAAALAAAAAABAAEAQAICRAEAOw==\" class=\"lazy\" data-original=\""+obj._defaults.jpgImageDomain+"/images/products/"+obj._defaults.imageSize[0]+"/" + feed.RECORDS[l].PROPERTIES.P_StyleSKU + "_"+obj._defaults.imageSize[1]+".jpg\" border=\"0\" />";
								} else {
									html += "<img src=\""+obj._defaults.jpgImageDomain+"/images/products/"+obj._defaults.imageSize[0]+"/" + feed.RECORDS[l].PROPERTIES.P_StyleSKU + "_"+obj._defaults.imageSize[1]+".jpg\" border=\"0\" />";
								}
							}else{
								//load up Png
								if(obj._defaults.lazyLoad) {
									html += "<img src=\"data:image/gif;base64,R0lGODlhAQABAID/AMDAwAAAACH5BAEAAAAALAAAAAABAAEAQAICRAEAOw==\" class=\"lazy\" data-original=\""+obj._defaults.imageDomain+"/is/image/EBFL2/"+ feed.RECORDS[l].PROPERTIES.P_StyleSKU + "?fmt=png-alpha&wid="+w+"&hei="+h+"\" border=\"0\" />";
								} else {
									html += "<img src=\""+obj._defaults.imageDomain+"/is/image/EBFL2/"+ feed.RECORDS[l].PROPERTIES.P_StyleSKU + "?fmt=png-alpha&wid="+w+"&hei="+h+"\" border=\"0\" />";
								}
							}
							testIMG = null; //Free memory
						}else{
							//Default JPG load
							if(obj._defaults.lazyLoad) {
								html += "<img src=\"data:image/gif;base64,R0lGODlhAQABAID/AMDAwAAAACH5BAEAAAAALAAAAAABAAEAQAICRAEAOw==\" class=\"lazy\" data-original=\""+obj._defaults.jpgImageDomain+"/images/products/"+obj._defaults.imageSize[0]+"/" + feed.RECORDS[l].PROPERTIES.P_StyleSKU + "_"+obj._defaults.imageSize[1]+".jpg\" border=\"0\" />";
			...
			//Fire callback
			if(typeof obj.callback == "function") obj.callback(); else {};
			//Do quickview if we're not liking to the PDP directly
			if(!obj._defaults.linkPDP){
				obj.setupLink();
			}
		}});
	...
	this.setupLink = function() {
		...
		$('body').on('click', 'a.product_cell', function() {
			...
			cmCreateManualLinkClickTag(window.location.host+'/?cm_re='+cm + '-_-' + obj.getcmstring($(this).attr('title')),obj.getcmstring($(this).attr('title')));
			return false;
		});
	},
...
function ReadMore() {
	...
	this.init = function(id) {
		...
		if($(obj.id.selector + " .extra_copy").index() > 0){	
			...
			html += '<div class="read_more readmore"><a href="javascript:void(0)" class="readmore" title="Read More">';
			if(typeof window['debug'] !== 'undefined'){
				console.log(html);
			}
			...
			html += '<div class="readless" style="display: none;"><a href="javascript:void(0);" class="readless" title="Hide">';
...
$.fn.ratings = function(options, callback) {
	$.ratings = new Ratings(); // rating instance
	$.ratings.id = $(this);
...
$.fn.collapse = function(options, callback) {
	$.collapse = new collapse(); // collapse instance
...
$.fn.dropdown = function(options, callback) {
	$.dropdown = new DropDown(); // dropdown instance
	$.dropdown.id = $(this);
					...
					for(var v = 0; v < data.DATA.length; v++) {
						if(data.DATA[v][vars.structure.CONTENTSOURCE] == 'partner') {
							...
							if(videoLink.indexOf('//www.youtube') !== -1) {
								...
								videoLink = $('#video_player_wrapper').youtube({'returnCode':'true','id':videoLink,'height':'315','width':'560'});
							}
							if(typeof window['debug'] !== 'undefined'){
								console.log('VIDEO'+videoLink);
					...
					for(var p = 0; p < productArray.length; p++) {
						if(p < settings.maxProducts){
							...
							if(settings.excludedSkus.indexOf(product[0]) === -1) {
								code = code + '<li><a href="'+settings.domain+'/product/model:'+product[1]+'/sku:'+product[0]+'" class="recentViewed" data-recentsku="'+product[0]+'"><img src="'+settings.imageDomain+'/pi/'+product[0]+'/'+settings.imageSize[0]+'/" border="0" /></a></li>';
							}
						} else {
...
(function($) { // Hide scope, no $ conflict
	...
	var methods = {
		init : function(options) {
			...
			return this.each(function(){
				var $element = $(this);
				$element.on('click', methods.loadFull);
		...
		initFull : function() {
			$('#full_screen').remove();
			$('body').prepend('<div id="full_screen"><div class="full_screen_header"><span class="inner_header"><span class="logo"></span><a href="#full_screen" class="close_btn"></a><span class="controls"><span id="zoomIn"></span><span class="zoom_label"></span><span id="zoomOut"></span></span></span></div><span class="main_view"></span><span class="alt_views"></span></div>');
			$('#full_screen').css({
				overflow :'hidden'
			});
		...
		loadFull : function() {
			...
			for(image in settings.views) {
				if(typeof window['debug'] !== 'undefined'){
					console.log(settings.views[image]);
				}
				$('<img />', {
		...
		zoomIn : function() {
			if(settings.currentZoom < settings.maxZoom) {
				...
				if(typeof window['debug'] !== 'undefined'){
					console.log($('#full_screen .main_view img.loading_img').attr('src'));
				}
				methods.loading();
		...
		loading : function() {
			...
			if(typeof window['debug'] !== 'undefined'){
				console.log(parseInt($('#full_screen .main_view').offset().left - (((settings.currentZoom * 500 - $('#full_screen .main_view img.loading_img').width()) / 2) )));
			}
	...
	var methods = {
		init : function(options) {
			var settings = {
				...
				hide : function (e, $el) { $el.fadeOut(100); if(typeof window['debug'] !== 'undefined'){ console.log('hide'); } },
				...
				$element.attr('id','addToWishlist');
					...
					$.getScript(settings.baseJS, function() {
						var data = '<script type="text/javascript">'
							...
							data += 'function showBubble(message) { if(typeof window[\'debug\'] !== \'undefined\'){console.log("Show Bubble"); } }';
						data += '<\/script>';  
						$('#wl_scripts').remove();
						$('<div />',{
							'id':'wl_scripts'
					...
					var data = '<script type="text/javascript">';
						data += 'function doAfterLogin() {';
							...
							data += 'dialogWidget.postSecureRequest(';
								...
								data += '}';
								data += ', function(){';
								data += '	if(typeof window[\'debug\'] !== \'undefined\'){ console.log("ERROR"); }';
								data += '}';
							data += ');';
						data += '}';
					data += '<\/script>';  
