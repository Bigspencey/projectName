...
{1;}
return this;};jQuery.fn.extend({selectbox:function(options){return this.each(function(){new jQuery.SelectBox(this,options);});}});jQuery.Autocompleter=function(input,options)
...
$(document).ready(function(){$("#tabCommunityStock").tabs({fxFade:true});$("#tabStockQuote").tabs({fxFade:true});});$(document).ready(function(){$('#ex2').jqm({ajax:'@href',ajaxClearBefore:true,trigger:'a.ex2trigger',toTop:true,modal:true,onLoad:RemoveViewStateField});$('#ex3').jqm({ajax:'@href',trigger:'a.ex3trigger',toTop:true,modal:true,overlayClass:'mediaWindow'});$('#ex4').jqm({overlay:0,trigger:false,ajax:'@href',overlayClass:'vidTutorial'});$('#ex5').jqm({ajax:'@rel',ajaxClearBefore:true,trigger:'a.ex5trigger',toTop:true,modal:true,onLoad:RemoveViewStateField});});function RemoveViewStateField(hash){$("input:hidden[id='__VIEWSTATE']",this).remove();$('a.helpRegister').cluetip({mouseOutClose:true,activation:'click',sticky:true,width:'357px',cluetipClass:'helpPopReg',dropShadow:false,showTitle:false,closePosition:'top',cluezIndex:1000,closeText:'<img src="http://www1.ibdcd.com/images/btn/helpPopupsCloseButton.gif" width="15" height="15" alt="close button" />'});UpdateGlossDefRollovers();}
function expandcollapse(obj){$('#'+obj).toggle();}
function modFsList_initCallback(carousel)
...
jQuery('#modFsList .jcarousel-control a').bind('click',function(){var pg=jQuery.jcarousel.intval(jQuery(this).text());carousel.scroll(pg);carousel.options.visiblePage=pg;carousel.options.fromPageDots=true;return false;});jQuery('#modFsList .jcarousel-next').bind('click',function(){if(carousel.options.visiblePage==jQuery('#modFsList .jcarousel-control a').length){jQuery('#modFsList .jcarousel-list').fadeOut('fast');carousel.options.animation=50;featuredCourseFade=true;}});};function modFsList_itemVisibleInCallbackAfterAnimation(carousel,item,idx,state)
...
if(prm!=null){prm.add_endRequest(EndRequest);}}});function EndRequest(sender,args){UpdateStockRollovers();UpdateGlossDefRollovers();InitMSTracking();InitNavTracking();}
...
$(document).ready(function(){$('.ttl-actions a').attr("title","Expand All");$('.ttl-actions a').toggle(function(){$(this).html("Collapse All &raquo;");$(this).attr("title","Collapse All");$('.currSubScript > .trigger').addClass('expanded').removeClass('trigger');$('.currSubScript > .hide').removeClass('hide').addClass('show');return false;},function(){$(this).html("Expand All &raquo;");$(this).attr("title","Expand All");$('.currSubScript > .expanded').addClass('trigger').removeClass('expanded');$('.currSubScript > .show').removeClass('show').addClass('hide');return false;});});function registerModalInit(){$('.toggle-expand').hide('fast');$("#mediaCtrTabs").tabs();$("#commentForm").hide();$(".commentWrite").click(function(){$("#commentsList").hide();$("#commentForm").show();});$("#cancelComment").click(function(){$("#commentForm").hide();$("#commentsList").show();});}
...
{return ft[1];}}
return'';}
var prevScrollTop=0;function AdjustScrollPosition(){if(!blnEnableScrolling&&$(document).scrollTop()>$("#divOverlayBottom").position().top-$(window).height()/1.3)
$(document).scrollTop(prevScrollTop);else if(!blnEnableScrolling)
prevScrollTop=$(document).scrollTop();else
prevScrollTop=0;}
...
s.dc=112
s.loadModule("Media")
s.Media.onLoad=function(s,m){s.Media.completeCloseOffsetThreshold=5;s.Media.segmentByMilestones=true;s.Media.trackSeconds=15;s.Media.trackUsingContextData=true;s.Media.contextDataMapping={"a.media.name":"eVar9,prop17","a.media.segment":"eVar12","a.contentType":"eVar18","a.media.timePlayed":"event1","a.media.view":"event4","a.media.segmentView":"event10","a.media.complete":"event5","a.media.milestones":{25:"event37",50:"event38",75:"event39"}};s.Media.trackMilestones="25,50,75";};s.m_Media_c="var m=s.m_i('Media');if(m.completeByCloseOffset==undefined)m.completeByCloseOffset=1;if(m.completeCloseOffsetThreshold==undefined)m.completeCloseOffsetThreshold=1;m.cn=function(n){var m="
...
+"addEventListener)s.wd.addEventListener('load',m.as,false);if(m.onLoad)m.onLoad(s,m)";s.m_i("Media");var s_code='',s_objectID;function s_gi(un,pg,ss){var c="s.version='H.25';s.an=s_an;s.logDebug=function(m){var s=this,tcf=new Function('var e;try{console.log(\"'+s.rep(s.rep(s.rep(m,\"\\\\\",\"\\\\\\"
...
return ajax.apply(this,arguments);};})(jQuery);(function(){var global=this;if(global.postscribe){return;}
