...
$(document).ready(function(){
	...
	//scroll to page content stopc
	if(iMobile && $('.stopc').length > 0)
	{
		$("html, body").animate({ scrollTop: $(".stopc").offset().top }, 1000);	
	}
	...
	$(".btnBC").click(function(){
		var cbw = $('.btnBCData').width();
		var cbh = $('.btnBCData').height();
		$.colorbox({
			iframe:true, 
			width:cbw, 
			height:cbh,
		...
		$.post(furl, {act:sllink.data("act"), pid:sllink.data("pid")}, function(d){
			...
			if(d.a == "success")
			{
				...
				if(d.t == "Remove from shortlist")
				{
					...
					slv.show();
					slt.html("Remove<br /><span class=\"hidden-xs visible-sm-inline\">from </span> Shortlist");
				}
				else
				{
